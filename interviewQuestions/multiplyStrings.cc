...
string subBitStrings( string first, string second , bool* nSign)
{
	string result; // To store the sum bits
    *nSign = 0;
	...
	// make the lengths same before adding
	int length = makeEqualLength(first, second);
	int reminder = 0; // Initialize reminder
	...
	for (int i = length-1 ; i >= 0 ; i--)
	{
		...
		// boolean expression for sum of 3 bits
		int diff = (firstBit - reminder - secondBit );
		if (diff < 0) {
		    reminder = 1;
		    diff = 10 + diff;
		    cout << "diff : " << diff << "\n";
...
{   int q = ((int)(a[0] - '0')*(b[0] - '0'))/10;
    int r = ((int)(a[0] - '0')*(b[0] - '0'))%10;
    
    if(q == 0) return to_string(r);
...
// 	printf ("%s\n", multiply("1100000000", "10100000000"));
// 	cout << multiply("0", "0") << "\n";
    bool nSign = 0;
	cout << subBitStrings("5","6", &nSign) << " ,"<< nSign;
// 	printf ("%s\n", multiply("11", "1010"));
// 	printf ("%s\n", multiply("1", "1010"));
// 	printf ("%s\n", multiply("0", "1010"));
// 	printf ("%s\n", multiply("111", "111"));
